...
Manager.prototype.start = function (cb) {
    ...
    this._socket = nssocket.createServer(function (socket) {
        ...
        socket.data(['host', 'remove'], function (data) {
            if (self._servers[data]) {
                var server = self._servers[data];
                self.removeServer(server);
                sendCommand(cluster.workers, {
                    cmd: 'removeServer',
                    params: [server.domain, server.port]
                });
                socket.send(['host', 'status'], JSON.stringify([undefined, "Removed"]));
            } else {
                socket.send(['host', 'status'], JSON.stringify(["Can't find server " + data]));
            }
        });
        socket.data(['host', 'removeall'], function () {
            self.clear();
            sendCommand(cluster.workers, {
                cmd: 'clear'
            });
        ...
        socket.data(['host', 'list'], function (data) {
            socket.send(['host', 'status'], JSON.stringify([undefined, self._servers.map(function (s, idx) {
                    return {
                        id: idx,
...
Manager.prototype.addServer = function (server, cb) {
    ...
    if (!this._ports["" + server.port] || !this._ports["" + server.port].length) {
...
Manager.prototype.removeServer = function (server) {
    var arr = this._servers;
    arr.splice(arr.indexOf(server), 1);

    arr = this._domains[server.domain];
    arr.splice(arr.indexOf(server), 1);

    arr = this._ports[server.port];
    arr.splice(arr.indexOf(server), 1);
};

Manager.prototype.removeByDomain = function (domain, callback) {
    var self = this;
    var servers = this._domains[domain];
...
Manager.prototype.removeByDomainPort = function (domain, port, callback) {
    var self = this;
    var toRemove = [];
